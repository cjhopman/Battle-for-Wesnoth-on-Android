...
int translate_x(int x) {
	...
	x = clamp(x, 0, SDL_ANDROID_sFakeWindWidth);
#else
...
int translate_y(int y) {
	...
	y = clamp(y, 0, SDL_ANDROID_sFakeWindHeight);
#else
...
{
	//__android_log_print(ANDROID_LOG_INFO, "libSDL", "SDL_ANDROID_nativeMouse(): (%d, %d), %d, %d", x, y, action, pointerId );
	...
	pointerId = clamp(pointerId, 0, MAX_MULTITOUCH_POINTERS);
...
extern void SDL_ANDROID_PumpEvents()
{
	...
	{
		__android_log_print(ANDROID_LOG_INFO, "libSDL", "SDL_ANDROID_MainThreadPumpEvent(): %d", BufferedEventsStart);
		...
		switch( ev.type )
		{
			case SDL_MOUSEMOTION:
				__android_log_print(ANDROID_LOG_INFO, "libSDL", "    MouseMotion: %d,%d", ev.motion.x, ev.motion.y);
				SDL_SendMouseMotion(NULL, 0, ev.motion.x, ev.motion.y);
				break;
			case SDL_MOUSEBUTTONDOWN:
				__android_log_print(ANDROID_LOG_INFO, "libSDL", "    MouseDown: %d,%d", ev.button.button, ev.button.state);
				if (ev.button.state == SDL_PRESSED || oldMouseButtons)
				{
					SDL_SendMouseButton( NULL, ev.button.state, ev.button.button );
				}
				oldMouseButtons = (ev.button.state == SDL_PRESSED);
				break;
			case SDL_KEYDOWN:
				__android_log_print(ANDROID_LOG_INFO, "libSDL", "    KeyDown: %d,%d", ev.key.keysym, ev.key.state);
...
static int getNextEvent()
{
	if( !BufferedEventsMutex )
		return -1;
	SDL_mutexP(BufferedEventsMutex);
	int nextEvent = BufferedEventsEnd + 1;
	if( nextEvent >= MAX_BUFFERED_EVENTS )
		nextEvent = 0;
	while( nextEvent == BufferedEventsStart )
	{
		__android_log_print(ANDROID_LOG_INFO, "libSDL", "Waiting for room in BufferedEvents");
		SDL_mutexV(BufferedEventsMutex);
...
extern void SDL_ANDROID_MainThreadPushMouseMotion(int x, int y)
{
	...
	__android_log_print(ANDROID_LOG_INFO, "libSDL", "SDL_ANDROID_MainThreadPushMouseMotion(): %d %d %d", x, y, nextEvent);
	if( nextEvent == -1 )
		return;
...
{
	int nextEvent = getNextEvent();
	__android_log_print(ANDROID_LOG_INFO, "libSDL", "SDL_ANDROID_MainThreadPushMouseButton(): %d %d %d", pressed, button, nextEvent);
	if( nextEvent == -1 )
		return;
