...
int ANDROID_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
	...
	for (i = 1; i < 4; i++) {
		SDL_modelist[i]->w = SDL_modelist[0]->w;
		SDL_modelist[i]->h = SDL_modelist[0]->h;
	}
...
static char ANDROID_APP_PACKAGE[] = STRINGIZE(SDL_JAVA_PACKAGE_PATH);

char* SDL_AndroidPackageName() {
