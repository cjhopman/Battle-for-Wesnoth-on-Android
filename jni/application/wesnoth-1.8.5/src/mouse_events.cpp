/* $Id: mouse_events.cpp 48450 2011-02-08 20:55:18Z mordante $ */
/*
...
void mouse_handler::mouse_motion(map_location new_hex, const bool browse, bool update) {
...
void mouse_handler::mouse_motion(int x, int y, const bool browse, bool update)
{
	...
	if (dragging_left_ && dragging_started_) {
		...
		if (dt > 50) {
			dx_ = static_cast<float>(dx) / dt;
			dy_ = static_cast<float>(dy) / dt;
			ticks_ += dt;
		}
...
void mouse_handler::left_mouse_up(int /*x*/, int /*y*/, const bool browse) {
	...
	if(!browse && !commands_disabled && attack_from.valid()) {
		if (move_ready_hex_ != last_hex_) {
			move_ready_hex_ = last_hex_;
			return;
		}
