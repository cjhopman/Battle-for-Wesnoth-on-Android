/* $Id: menu_events.cpp 49342 2011-05-02 18:52:36Z crab $ */
/*
...
void menu_handler::unit_list()
{
	...
	int selected = -1;
	foreach(const unit_map::iterator::value_type& unit, units_) {
		if (unit.second.side() != gui_->viewing_side())
			continue;
		if (gui_->selected_hex() == unit.first) {
			selected = units_list.size();
		}
		units_list.push_back(unit.second);
	}
	if (gui2::new_widgets) {
		gui2::tunit_list dlg(units_list, selected);

		dlg.show(gui_->video());

		if(dlg.get_retval() == gui2::twindow::OK) {
			selected = dlg.get_selected_unit();
		} else {
			selected = -1;
		}
	} else {
	...
	items.push_back(heading);
		...
		/** @todo TODO: hero just has overlay "misc/hero-icon.png" - needs an ability to query */
			...
			row << unit.type_name() << COLUMN_SEPARATOR;
			if(unit.can_recruit() ) {
			row << "<205,173,0>";   // gold3
		}
			row << unit.name()   << COLUMN_SEPARATOR;

		// display move left (0=red, moved=yellow, not moved=green)
			if(unit.movement_left() == 0) {
			row << font::RED_TEXT;
			} else if(unit.movement_left() < unit.total_movement() ) {
			row << "<255,255,0>";
		} else {
			row << font::GREEN_TEXT;
		}
			row << unit.movement_left() << "/" << unit.total_movement() << COLUMN_SEPARATOR;
		...
		// show icons if unit is slowed, poisoned, petrified, invisible:
			...
			if(unit.invisible(unit.get_location(), units_, teams_ ,false))
			row << IMAGE_PREFIX << "misc/invisible.png";
		row << COLUMN_SEPARATOR;

		// Display HP
		// see also unit_preview_pane in dialogs.cpp
			row << font::color2markup(unit.hp_color());
			row << unit.hitpoints()  << "/" << unit.max_hitpoints() << COLUMN_SEPARATOR;
		...
		// Display XP
			row << font::color2markup(unit.xp_color());
			row << unit.experience() << "/";
			if(unit.can_advance()) {
				row << unit.max_experience();
		} else {
			row << "-";
		}
		row << COLUMN_SEPARATOR;
	...
	if(selected >= 0 && selected < int(units_list.size())) {
		const map_location& loc = units_list[selected].get_location();
...
void menu_handler::preferences(bool use_classic)
{
	if (use_classic) {
	preferences::show_preferences_dialog(*gui_, game_config_);
	gui_->redraw_everything();
	} else {
		gui2::tpreferences dlg(&game_config_);
		dlg.show(gui_->video());
	}
}
...
void menu_handler::recruit(bool browse, int side_num, const map_location &last_hex)
{
	...
	foreach(const std::string& str, current_team.recruits()) {
		const unit_type *type = unit_types.find(str);
		if (!type) {
			ERR_NG << "could not find unit '" << str << "'\n";
			return;
		}
	...
	if(gui2::new_widgets) {
		gui2::tunit_recruit dlg(teams_[side_num - 1], sample_units);

		dlg.show(gui_->video());

		if(dlg.get_retval() == gui2::twindow::OK) {
			recruit_res = dlg.get_selected_recruit();
		} else {
			recruit_res = -1;
		}
	} else {
		std::vector<std::string> items;
		foreach(const unit_type* type, sample_units) {
	...
	if(recruit_res != -1) {
		std::set<std::string>::const_iterator it = current_team.recruits().begin();
		std::advance(it, recruit_res);
		do_recruit(*it, side_num, last_hex);
	}
}
...
void menu_handler::recall(int side_num, const map_location &last_hex)
{
	...
	if(gui2::new_widgets) {
		gui2::tunit_recall dlg(teams_[side_num - 1]);

		dlg.show(gui_->video());

		if(dlg.get_retval() == gui2::twindow::OK) {
			res = dlg.get_selected_unit();
		} else {
			res = -1;
		}
	} else {
