/* $Id: construct_dialog.cpp 48450 2011-02-08 20:55:18Z mordante $ */
/*
...
int dialog::process(dialog_process_info &info)
{
	...
	info.new_key_down = info.key[SDLK_SPACE] || info.key[SDLK_RETURN] ||
					info.key[SDLK_ESCAPE] || info.key[SDLK_KP_ENTER] || info.key[SDLK_UNDO];
	...
	if(!info.key_down && (info.key[SDLK_ESCAPE] || info.key[SDLK_UNDO]) && !(type_ == OK_ONLY && has_input)) {
		return (CLOSE_DIALOG);
	}
