/* $Id: display.hpp 48934 2011-03-19 19:29:54Z boucman $ */
/*
...
public:
	...
	void inertial_scroll(float dx, float dy);
...
protected:
	 ...
	 */
	static int get_group(int l) {
		static const display::tdrawing_layer layer_groups[] = {
			...
			display::LAYER_REACHMAP,
			display::LAYER_LAST_LAYER
		};
		return std::upper_bound(layer_groups, layer_groups + 5, l) - layer_groups - 1;
	}
...
public:
	struct tblit2 {
		...
		tblit2()
		{ }
		tblit2(int x, int y, map_location h, display::tdrawing_layer l, surface s, SDL_Rect r) :
			x(x), y(y),
			group(get_group(l)),
			layer(l),
			surf(s),
	...
	typedef std::vector<tblit2> tdrawing_buffer;
