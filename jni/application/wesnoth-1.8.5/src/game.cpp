/* $Id: game.cpp 49411 2011-05-07 15:11:50Z soliton $ */
/*
...
#if !defined(ANDROID)
std::string SDL_AndroidPackageName() { return "full"; }
#endif
...
bool game_controller::new_campaign()
{
	...
	if(campaigns.begin() == campaigns.end()) {
		std::string package_name(SDL_AndroidPackageName());
		t_string message;
		if (package_name.substr(package_name.size() - 4, 4) == "free") {
			message = _("The full version of Battle for Wesnoth contains over 200 scenarios across 15 campaigns. Get it now in the Android Market.\n");
		} else {
			message = _("License verification failed. Contact support@androthsoft.com for more information.");
		}
		gui2::show_message(
				disp().video(),
				_(""),
...
static int do_gameloop(int argc, char** argv)
{
	...
	for(;;){
		...
		if (res == gui::NOTHING && gui2::new_widgets) {
			...
			cursor::set(cursor::NORMAL);
			gui2::ttitle_screen dlg(loadscreen_manager);
