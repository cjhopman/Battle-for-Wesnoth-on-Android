...
#include "log.hpp"
#include "map.hpp"
#include "multiplayer_connect_base.hpp"
...
void connect::side::resolve_random()
{
			...
			false))
	{
		...
		sort(faction_choices.begin(), faction_choices.end());
		...
		sort(faction_excepts.begin(), faction_excepts.end());
		...
		foreach (const config *i, parent_->era_sides_)
		{
			...
			if ((*i)["random_faction"] != "yes") {
				...
				if (!std::binary_search(faction_choices.begin(),faction_choices.end(),faction_id))
					continue;
				if (std::binary_search(faction_excepts.begin(),faction_excepts.end(),faction_id))
					continue;
...
connect::connect(const config& game_config,
...
void connect::process_network_error(network::error& error)
{
	...
	connected_user_list::iterator user = std::find_if(users_.begin(), users_.end,
			bind(&connected_user::connection) == error.socket);
	if(user != users_.end()) {
		changes = true;
...
void connect::load_game()
{
	if(params_.saved_game) {
		savegame::loadgame load(disp_, game_config_, state_);
...
void connect::update_and_send_diff(bool update_time_of_day)
{
	...
	if (update_time_of_day)
	{
		...
		tod_manager tod_mng(level_, lexical_cast<int>(level_["turns"]), &state_);
	}
...
{
	return std::find_if(sides_.begin(), sides_.end(), (!bind(&side::ready_for_start, _1)))
		== sides_.end();
}

bool connect::sides_available() const
{
	return std::find_if(sides_.begin(), sides_.end(), bind(&side::available, _1))
		!= sides_.end();
}
...
bool connect::can_start_game() const
{
	 ...
	 */
	return std::find_if(sides_.begin(), sides_.end(),
...
void connect::update_playerlist_state(bool silent)
{
	...
	if (gamelist().child("user") != NULL) {
		...
		foreach(connected_user& u, users_) {
			playerlist.push_back(u.name);
		}
...
int connect::find_player_side(const std::string& id) const
{
	...
	return itor == sides_.end() ? -1 : itor - sides_.begin();
}
