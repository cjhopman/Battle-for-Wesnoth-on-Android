/* $Id: display.cpp 48934 2011-03-19 19:29:54Z boucman $ */
/*
...
bool operator<(const display::tblit2& l, const display::tblit2& r) {
	return boost::make_tuple(l.group, l.loc.y, l.loc.x & 1, l.layer, l.loc.x)
		< boost::make_tuple(r.group, r.loc.y, r.loc.x & 1, r.layer, r.loc.x);
}


void display::drawing_buffer_add(const tdrawing_layer layer,
		const map_location& loc, const tblit& blit) {
	foreach (const surface& s, blit.surf) {
		drawing_buffer_.push_back(tblit2(blit.x, blit.y, loc, layer, s, blit.clip));
	}
}
...
void display::drawing_buffer_commit()
{
	...
	std::stable_sort(drawing_buffer_.begin(), drawing_buffer_.end());

	foreach(const tblit2& t, drawing_buffer_) {
		if (t.surf.null()) continue;

		SDL_Rect dstrect = { t.x, t.y, 0, 0 }, srcrect = t.rect;

		SDL_BlitSurface(t.surf,
					srcrect.x || srcrect.y || srcrect.w || srcrect.h ? &srcrect : NULL,
					dst, &dstrect);
		update_rect(t.x, t.y, zoom_, zoom_);
	}
...
void display::inertial_scroll(float dx, float dy) {
	dx_ = dx;
	dy_ = dy;
	ticks_ = SDL_GetTicks();
}
...
//	log_scope("display::draw");
	...
	if (dx_ || dy_) {
		...
		if (dt > 20) {
			float ndx, ndy, sx, sy;
			boost::tie(ndx, sx) = inertia::decay(dx_, dt);
			boost::tie(ndy, sy) = inertia::decay(dy_, dt);
