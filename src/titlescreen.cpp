/* $Id: titlescreen.cpp 48450 2011-02-08 20:55:18Z mordante $ */
/*
...
TITLE_RESULT show_title(game_display& screen, config& tips_of_day)
{
	...
	SDL_Rect main_dialog_area = {menu_xbase-padding, menu_ybase-padding, max_width+padding*2,
								 menu_yincr*(n_menubuttons)+buttons[QUIT_GAME - TUTORIAL].height()+padding*2};
	...
	for(;;) {
		...
		if (key[SDLK_MENU] || key[SDLK_p]) {
			return static_cast<TITLE_RESULT>(EDIT_PREFERENCES);
		}
