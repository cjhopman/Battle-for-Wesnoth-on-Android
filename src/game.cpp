...
bool game_controller::load_game()
{
	...
	ERR_GENERAL << 12 << std::endl;
	recorder = replay(state_.replay_data);
	recorder.start_replay();
	recorder.set_skip(false);
	ERR_GENERAL << 13 << std::endl;
...
bool game_controller::new_campaign()
{
	...
	if(campaigns.begin() == campaigns.end()) {
		gui2::show_message(
				disp().video(),
				_(""),
				_("The full version of Battle for Wesnoth contains over 200 scenarios across 15 campaigns. Get it now in the Android Market.\n"),
...
bool game_controller::play_multiplayer()
{
	gui2::show_message(
			disp().video(),
			_(""),
			_("Multiplayer is not available during the beta period."),
			gui2::tmessage::ok_button,
			true);
	return false;
